...
use backupdbtool::config::{CosProvider, get_all_config, get_webhook, resolve_password};
...
async fn main() -> Result<()> {
    ...
    // 加载配置
    let password = match resolve_password(cli.password, cli.password_file) {
        Ok(password) => password,
        Err(e) => {
            error!("Failed to resolve password: {}", e);
            process::exit(1);
        }
    };
    ...
    let storage = match app_config.storage(&config).await {
        Ok(storage) => storage,
        Err(e) => {
    ...
    match cli.command {
        ...
        Commands::Delete { key, all } => {
            ...
            utils::cleanup_old_backups(&app_config.get_backup_dir(), app_config.retention_days)
                .await?;
            delete_from_cos(
                key,
                all,
